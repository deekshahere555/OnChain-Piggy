...
function Features() {
  const cards = [
    { icon: '💎', title: 'Deposit FLOW', desc: 'Lock native FLOW (EVM) with one click using depositETH().' },
...
function Story() {
  const steps = [
    { title: 'Connect Wallet', desc: 'Open the app and connect MetaMask on Flow EVM Testnet.', icon: '🔌' },
    { title: 'Deposit Tokens', desc: 'FLOW via depositETH or any ERC-20 via approve + depositToken.', icon: '💰' },
    ...
    <footer id="faq" className="border-t border-white/10 py-10">
      <div className="mx-auto max-w-6xl px-6 grid md:grid-cols-3 gap-8 text-sm text-white/70">
        <div>
          ...
          <p className="mt-2">Timelock savings on Flow EVM Testnet — ETH/Flow & ERC-20 friendly.</p>
        </div>
        <div>
...
function VaultPreview() {
  return (
    <div className="grid sm:grid-cols-2 gap-4">
      <Card title="💎 Deposit FLOW" lines={['Amount: 0.05 FLOW', 'Lock: 300s', 'Status: Ready to sign']} />
